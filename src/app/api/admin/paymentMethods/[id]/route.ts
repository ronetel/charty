...
export async function PUT(req: Request, { params }: any) {
  ...
  try {
    ...
    try {
      ...
      details.diff = diff;
      await createAuditLog({ userId: payload?.id, action: 'update', entity: 'payment_method', entityId: updated.id, details });
    } catch (e) {}
...
export async function DELETE(req: Request, { params }: any) {
  ...
  try {
    const payload = verifyToken(token || '');
    await createAuditLog({ userId: payload?.id, action: 'delete', entity: 'payment_method', entityId: id, details: { before } });
  } catch (e) {}
